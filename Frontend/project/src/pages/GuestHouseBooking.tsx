...
interface GuestHouse {
    ...
    images: string[];
    rooms: number;
    // Optional extended fields if your API returns them
    location?: string;
    price?: number;
    rating?: number;
    amenities?: string[];
...
const GuestHouseBooking: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProperties = async () => {
            try {
                // Use the correct endpoint path (matching your working code)
                const response = await fetch('http://localhost:5000/api/gproperties');
                const data = await response.json();

                // If your API returns data.properties directly, use that
